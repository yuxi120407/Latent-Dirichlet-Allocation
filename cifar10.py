# -*- coding: utf-8 -*-
"""
Created on Thu Oct 18 11:05:10 2018
...
model = VGG16(10, nn.ReLU())                                   # Note that if you run this scipt on a cpu you must remove the ".cuda()"
                                    # from this line.
...
outputs, layers = model(inputs.cpu()) # Get the ouput of the network and each layer before any training.
...
for epoch in range(10):     # Run for 10 epochs
    ...
    inputs, labels = dataiter.next()
    outputs, layers = model(inputs.cpu())
    ...
    for i, data in enumerate(trainloader, 0): # Run through entire training set.
        ...
        outputs, layers = model(inputs.cpu())  # If you wnat to run on cpu
        loss = criterion(outputs, labels.cpu())# remove ".cuda()"
        loss.backward()
        optimizer.step()
